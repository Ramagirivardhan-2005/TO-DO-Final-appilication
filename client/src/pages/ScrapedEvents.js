...
export default function ScrapedEvents({ user }) {
  ...
  useEffect(() => {
    const checkConfig = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/email/config/${user.id}`);
  ...
  const fetchEvents = useCallback(async () => {
    ...
    setError(null);
    try {
      let url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/scrape/all?userId=${user.id}`;
      if (activeFilter === 'Codeforces') url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/scrape/codeforces`;
      if (activeFilter === 'LeetCode') url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/scrape/leetcode`;
      if (activeFilter === 'CodeChef') url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/scrape/codechef`;
      if (activeFilter === 'Hackathons') url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/scrape/hackathons`;
      if (activeFilter === 'Email') url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/email/scan/${user.id}`;
      if (activeFilter === 'Instagram') url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/scrape/instagram-all/${user.id}`;
  ...
  const importAsTask = async (event) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/import-task`, {
        user_id: user.id,
        name: event.name,
  ...
  const saveSettings = async () => {
    ...
    setSettingsMsg('');
    try {
      await axios.post(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/email/config`, {
        user_id: user.id,
