...
export default function ScrapedEvents({ user }) {
  ...
  useEffect(() => {
    const checkConfig = async () => {
      try {
        const res = await axios.get(`${API_URL}/api/email/config/${user.id}`);
  ...
  const fetchEvents = useCallback(async () => {
    ...
    setError(null);
    try {
      let url = `${API_URL}/api/scrape/all?userId=${user.id}`;
      if (activeFilter === 'Codeforces') url = `${API_URL}/api/scrape/codeforces`;
      if (activeFilter === 'LeetCode') url = `${API_URL}/api/scrape/leetcode`;
      if (activeFilter === 'CodeChef') url = `${API_URL}/api/scrape/codechef`;
      if (activeFilter === 'Hackathons') url = `${API_URL}/api/scrape/hackathons`;
      if (activeFilter === 'Email') url = `${API_URL}/api/email/scan/${user.id}`;
      if (activeFilter === 'Instagram') url = `${API_URL}/api/scrape/instagram-all/${user.id}`;
  ...
  const importAsTask = async (event) => {
    ...
    try {
      await axios.post(`${API_URL}/api/import-task`, {
        user_id: user.id,
        name: event.name,
  ...
  const saveSettings = async () => {
    ...
    setSettingsMsg('');
    try {
      await axios.post(`${API_URL}/api/email/config`, {
        user_id: user.id,
