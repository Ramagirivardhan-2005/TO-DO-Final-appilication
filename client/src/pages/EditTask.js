...
export default function EditTask() {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/task/${id}`) 
      .then(res => {
        const formattedData = {
            ...res.data,
        ...
        if(currentUser) {
            axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/tasks/${currentUser.id}`).then(res => {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    let url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/tasks/${id}`;
