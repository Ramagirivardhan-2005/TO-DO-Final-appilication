...
export default function EditTask() {
  ...
  useEffect(() => {
    axios.get(`${process.env.REACT_APP_API_URL || "https://to-do-final-appilication-1.onrender.com"}/task/${id}`) 
      .then(res => {
        const formattedData = {
            ...res.data,
        ...
        if(currentUser) {
            axios.get(`${process.env.REACT_APP_API_URL || "https://to-do-final-appilication-1.onrender.com"}/tasks/${currentUser.id}`).then(res => {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    let url = `${process.env.REACT_APP_API_URL || "https://to-do-final-appilication-1.onrender.com"}/tasks/${id}`;
