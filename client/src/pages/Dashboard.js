...
export default function Dashboard({ user, logout }) {
  ...
  const fetchTasksAndUser = useCallback(async () => {
    try {
      ...
      const resT = await axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/tasks/${user.id}`);
      setTasks(resT.data);

      // Fetch User points
      const resU = await axios.get(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/users/${user.id}`);
  ...
  const handleToggleComplete = async (task) => {
    try {
      const res = await axios.put(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/tasks/${task._id}/complete`);
  ...
  const handleDelete = async (task) => {
    let url = `${process.env.REACT_APP_API_URL || "http://localhost:5000"}/tasks/${task._id}`;
    if (task.recurring_id) {
  ...
  const handleMfaSetup = async () => {
    ...
    setMfaMsg({ text: '', type: '' });
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/mfa/setup`, { userId: user.id });
  ...
  const handleMfaVerifySetup = async () => {
    ...
    setMfaMsg({ text: '', type: '' });
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/mfa/verify-setup`, {
        userId: user.id,
        token: mfaCode
      });
      if (res.data.success) {
  ...
  const handleMfaDisable = async () => {
    ...
    setMfaMsg({ text: '', type: '' });
    try {
      const res = await axios.post(`${process.env.REACT_APP_API_URL || "http://localhost:5000"}/api/mfa/disable`, {
        userId: user.id,
        token: mfaCode
      });
      if (res.data.success) {
