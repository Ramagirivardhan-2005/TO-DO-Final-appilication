...
export default function Wallet({ user, setUser }) {
    ...
    const fetchWalletData = useCallback(async () => {
        try {
            const [balRes, txRes, subRes] = await Promise.all([
                ...
                axios.get(`http://localhost:5000/api/wallet/transactions/${user.id}`),
                axios.get(`http://localhost:5000/api/wallet/subscriptions/${user.id}`)
            ]);
    ...
    const handleRedeem = async () => {
        ...
        setLoading(true);
        try {
            const res = await axios.post('http://localhost:5000/api/wallet/redeem', {
                userId: user.id,
    ...
    const handleSubscribe = async (service) => {
        ...
        setLoading(true);
        try {
            const res = await axios.post('http://localhost:5000/api/wallet/subscribe', {
                userId: user.id,
